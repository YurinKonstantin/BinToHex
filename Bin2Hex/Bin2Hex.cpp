...
HWND headerOffsetText;//текстовый контрол
HWND headerbyteText;//текстовый контрол
HWND headerASNText;//текстовый контрол
                     ...
                     _In_ int       nCmdShow)
{
   ...
   // PCWSTR languages[] = { L"en-US" };

    // Задаем приоритетный язык пользовательского интерфейса для текущего потока
   //SetThreadPreferredUILanguages(MUI_LANGUAGE_NAME, L"hi-IN", NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        hHexView = CreateHexView(hWnd);//Окно отображения файла по байтам
        ...
        hInfo = CreateWindow(
            ...
            0, 16, 350, 200,                // X, Y, Ширина, Высота
            hWnd,                    // Дескриптор родительского окна
            NULL,                          // ID элемента (можно задать свой)
            NULL,                         // Дескриптор приложения
            NULL                           // Дополнительные данные
