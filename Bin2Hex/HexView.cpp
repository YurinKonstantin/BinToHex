...
HWND CreateHexView(_In_ HWND hWndParent);
LRESULT CALLBACK HexViewProc(_In_ HWND hWnd, _In_ UINT message, _In_ WPARAM wParam, _In_ LPARAM lParam);
VOID HexView_SetPositionOfColumns(_Inout_ PHEXVIEW HexView);
VOID HexView_SendNotify(_In_ HWND hWnd, _In_ UINT Code, _In_ LPNMHDR NmHdr);
_Check_return_
BOOL
HexView_SetNumberOfItem(_Inout_ PHEXVIEW HexView, _In_ int xPos, _In_ int yPos);
VOID HexView_SetCaret(_In_ HWND hWnd, _Inout_ PHEXVIEW HexView);
VOID HexView_SetScrollInfo(_In_ HWND hWnd, _In_ PHEXVIEW HexView);
VOID HexView_Paint(_In_ HWND hWnd, _In_ HDC hdc, _In_ PHEXVIEW HexView);
VOID HexView_DrawLine(_In_ HWND hWnd, _In_ HDC hdcMem, _In_ PHEXVIEW HexView, _In_ int NumberOfLine);
_Check_return_ LONG_PTR HexView_GetTrackPos(_In_ HWND hWnd, _In_ PHEXVIEW HexView, _In_ int nBar);
_Check_return_ BOOL HexView_PinToBottom(_Inout_ PHEXVIEW HexView);
...
Скрытие происходит накопительно.

Если ваше приложение вызывает HideCaret пять раз подряд,
оно также должно вызвать ShowCaret пять раз, прежде чем курсор отобразится.
Поэтому нам нужен IsCaretVisible, чтобы скрыть курсор только один раз.
...
_Check_return_ BOOL HexView_SetNumberOfItem(_Inout_ PHEXVIEW HexView, _In_ int xPos, _In_ int yPos)
{
...
LRESULT CALLBACK HexViewProc(_In_ HWND hWnd,_In_ UINT message, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
    {
        ...
        if (HexView_SetNumberOfItem(HexView, xPos, yPos)) {
            ...
            NMHVDISPINFO DispInfo = { 0 };
            // DispInfo.Item. = HexView->TotalLines;
            DispInfo.Item.NumberOfItem = HexView->NumberOfItem;


            HexView_SendNotify(hWnd, HVN_StaticText, (LPNMHDR)&DispInfo);
        }
