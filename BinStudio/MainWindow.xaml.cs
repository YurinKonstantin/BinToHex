...
namespace BinStudio
{
    ...
    public sealed partial class MainWindow : Window
    {
        ...
        // Вызывается при нажатии "Правка" -> "Найти..." в верхнем меню
        private void OpenSearchPanel_Click(object sender, RoutedEventArgs e)
        {
            if (DocTabView.SelectedItem is TabContext activeTab)
            {
                activeTab.IsSearchPanelOpen = true; // Панель откроется
            }
        }

        // Вызывается при нажатии кнопки крестика (Закрыть) вверху боковой панели
        private void CloseSearchPanel_Click(object sender, RoutedEventArgs e)
        {
            if (DocTabView.SelectedItem is TabContext activeTab)
            {
                activeTab.IsSearchPanelOpen = false; // Панель скроется
            }
        }
